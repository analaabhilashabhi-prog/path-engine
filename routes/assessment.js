...
router.post('/generate-quiz', async (req, res) => {
  try {
    ...
    var prompt = 'You are a strict assessment AI. Generate exactly 20 UNGOOGLEABLE quiz questions for a ' + (education || '') + ' student studying ' + domain + '.\n\nStudent self-rated: ' + ratingsSummary + '\n\nCRITICAL RULES:\n- Skills rated 4-5: HARD questions to verify their claim\n- Skills rated 3: MEDIUM questions\n- Skills rated 1-2: EASY questions but still scenario-based\n- EVERY question MUST be ungoogleable. NO definition questions. NO "what does X stand for" questions.\n- Question types to use:\n  * Scenario: "A developer does X, what happens next?"\n  * Code output: "What does this code print?" (show small code snippet in the question)\n  * Debugging: "This code has a bug, what is it?"\n  * Decision: "You need to build X feature, which approach is best and why?"\n  * Tradeoff: "Compare approach A vs B for this situation"\n  * What-if: "If you change X in this code, what breaks?"\n- Each question must require UNDERSTANDING, not memorization\n- All questions must have exactly 4 options\n- Make wrong options plausible (not obviously wrong)\n- Shuffle difficulty throughout, dont group easy/medium/hard\n\nReturn ONLY a JSON array of 20 objects:\n[{"id":1,"type":"scenario","difficulty":"easy","skill":"HTML","question":"A developer adds a <div> inside a <p> tag. What happens when the browser renders this?","options":["The browser auto-closes the <p> and creates two separate elements","Nothing, it renders normally","The page crashes with a syntax error","The <div> becomes inline automatically"],"correct":0,"time":30}]\n\n- correct = index (0-3) of right answer\n- time = 30 for easy, 45 for medium, 60 for hard\n- type = "scenario", "code_output", "debugging", or "decision"\n- Return ONLY the JSON array. No markdown. No explanation. No backticks.';
